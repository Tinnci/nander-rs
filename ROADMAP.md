...
|------|---------|-----------|
| SPI NAND Flash | ✅ 85 款芯片 | 🔶 16 款芯片 |
| SPI NOR Flash | ✅ 146 款芯片 | 🔶 14 款芯片 |
...
*   **性能飞跃**: ⏳ 待优化 - 优化基础设施层的批量传输逻辑，提升读写吞吐量。

### Phase 2: EEPROM 与 生态扩展 (v0.4.0)
*(后续按计划进行...)*
...
**扩展芯片数据库**:
- [ ] 从 SNANDer 迁移所有 85 款 NAND 芯片
- [ ] 从 SNANDer 迁移所有 146 款 NOR 芯片
- [ ] 添加芯片参数验证

---

### Phase 2: EEPROM 支持 (v0.3.0)
...
3. **扩展芯片数据库** ⏳ 进行中
   - [ ] 从 SNANDer 的 `spi_nand_flash.c` 提取芯片定义
   - [ ] 从 SNANDer 的 `spi_nor_flash.c` 提取芯片定义

4. **下一步任务** (后续迭代)
   - ✅ 实现坏块检测和跳过 (-k / -K 选项)
   - ✅ 添加 OOB 区域读写支持 (-o / -O 选项)
   - [ ] 实现 ECC 错误忽略模式 (-I)
   - [ ] 添加详细进度条 (速度统计, 预计剩余时间)
   - [ ] 添加单元测试覆盖
